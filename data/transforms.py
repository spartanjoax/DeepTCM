...
"""Signal normalisation and scalogram transforms for the TCM data pipeline.

Implements StdScalerTransform, MinMaxScalerTransform, StdScaler3D, and the
STFT scalogram transform used as the champion preprocessing front-end.
"""
import torch
...
class StdScalerTransform(torch.nn.Module):
    def __init__(self, mean, std):
        """
        Custom transform to normalize signals using z-score normalisation.
    ...
    def forward(self, tensor: torch.Tensor):
        """
        Apply normalisation to the sample.
        ...
        Returns:
            torch.Tensor: Normalised tensor of the same shape as the input.
        """
    ...
    def __repr__(self) -> str:
        """Return a string representation of the transform with its fitted parameters."""
